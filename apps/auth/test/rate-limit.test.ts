...
describe('auth rate limiting', () => {
  it('enforces a fixed window and resets without extending it', () => {
    ...
    expect(
      limiter.consume('account', { max: 2, windowSeconds: 60 }).allowed,
    ).toBe(true);
    expect(
      limiter.consume('account', { max: 2, windowSeconds: 60 }).allowed,
    ).toBe(true);
  ...
  it('uses five-per-minute limits for authentication and verification', () => {
    expect(authRateLimitPolicy('/api/auth/sign-in/email')).toEqual({
      max: 5,
      windowSeconds: 60,
    });
    ...
    expect(authRateLimitPolicy('/api/auth/account/deletion/recovery').max).toBe(
      5,
    );
    expect(authRateLimitPolicy('/api/auth/mfa/step-up/totp').max).toBe(5);
    expect(authRateLimitPolicy('/api/auth/connect-password').max).toBe(5);
  });
  ...
  it('ignores attacker-controlled junk cookies when selecting the stable quota key', () => {
    const common = {
    ...
    expect(
      authRateLimitKey({
        ...common,
        cookieHeader: 'rsp-auth.session_token=signed; junk=one',
      }),
    ).toBe(
      authRateLimitKey({
        ...common,
        cookieHeader:
          'attacker=random; rsp-auth.session_token=signed; junk=two',
      }),
    );
    expect(authRateLimitKey({ ...common, cookieHeader: 'junk=one' })).toBe(
      authRateLimitKey({ ...common, cookieHeader: 'junk=two' }),
    );
  });
});
