...
import { evaluateExplicitLink, evaluateFreshSession } from "../src/security/linking-policy.js";
...
describe("explicit account linking", () => {
  ...
  it("rejects implicit/stale and unauthenticated linking", () => {
    expect(evaluateExplicitLink({ ...baseline, authenticatedUserId: undefined }).reason).toBe(
      "not_authenticated",
    );
    expect(
      evaluateExplicitLink({ ...baseline, sessionCreatedAt: "2026-08-13T11:54:59.000Z" }).reason,
  ...
  it("reuses the same fresh-session rule for MFA and backup-code management", () => {
    ...
    expect(
      evaluateFreshSession({ ...baseline, sessionCreatedAt: "2026-08-13T11:00:00.000Z" }).reason,
    ).toBe("reauthentication_required");
  });

  it("allows no provider other than Google", () => {
    expect(evaluateExplicitLink({ ...baseline, provider: "github" }).reason).toBe(
  ...
  it("never links different emails", () => {
    expect(
      evaluateExplicitLink({ ...baseline, providerEmail: "attacker@example.org" }).reason,
    ).toBe("different_email");
    expect(
      evaluateExplicitLink({ ...baseline, providerEmail: "MEMBER@example.org" }).allowed,
    ).toBe(true);
  });
});
