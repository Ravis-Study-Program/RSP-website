...
describe('account lifecycle policy', () => {
  it('supports suspension and the 30-day deletion progression', () => {
    ...
    expect(nextAccountState('active', 'request_deletion')).toBe(
      'deletion_pending',
    );
    expect(nextAccountState('deletion_pending', 'cancel_deletion')).toBe(
      'active',
    );
    expect(nextAccountState('deletion_pending', 'pseudonymize')).toBe(
      'deleted',
    );
  });

  it('rejects irreversible or semantically invalid transitions', () => {
    expect(() => nextAccountState('deleted', 'cancel_deletion')).toThrow(
      /invalid/,
    );
