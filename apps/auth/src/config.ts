...
const environmentSchema = z
  .object({
    ...
    DATABASE_URL: z
      ...
      .url()
      .default(
        'postgresql://rsp:rsp@localhost:5432/rsp?options=-c%20search_path%3Dauth',
      ),
    ...
    AUTH_MFA_MAX_AGE_SECONDS: z.coerce
      .number()
      .int()
      .min(60)
      .max(3_600)
    ...
    IDENTITY_SERVICE_URL: z.url().default('http://localhost:4000'),
    IDENTITY_SERVICE_TOKEN: z.string().min(32).default(LOCAL_IDENTITY_TOKEN),
    IDENTITY_CALLBACK_TIMEOUT_MS: z.coerce
      .number()
      .int()
      .min(100)
      .max(30_000)
  ...
  .superRefine((environment, context) => {
    ...
    if (baseUrl.pathname !== '/' || baseUrl.search || baseUrl.hash) {
      context.addIssue({
        code: 'custom',
        message:
          'BETTER_AUTH_URL must be an origin without a path, query, or fragment',
    ...
    if (environment.NODE_ENV === 'production') {
      ...
      if (!environment.AUTH_TRUST_PROXY_HEADERS) {
        context.addIssue({
          code: 'custom',
          message: 'AUTH_TRUST_PROXY_HEADERS must be true in production',
          path: ['AUTH_TRUST_PROXY_HEADERS'],
        });
      }
      if (environment.AUTH_IP_ADDRESS_HEADER.toLowerCase() !== 'x-real-ip') {
        context.addIssue({
          code: 'custom',
          message: 'AUTH_IP_ADDRESS_HEADER must be x-real-ip in production',
          path: ['AUTH_IP_ADDRESS_HEADER'],
        });
      }
      if (
        environment.BETTER_AUTH_SECRET === environment.IDENTITY_SERVICE_TOKEN
      ) {
        context.addIssue({
          code: 'custom',
          message: 'IDENTITY_SERVICE_TOKEN must not reuse BETTER_AUTH_SECRET',
          path: ['IDENTITY_SERVICE_TOKEN'],
        });
      }
      ...
      if (environment.EMAIL_FROM.toLowerCase().includes('rsp.local')) {
        context.addIssue({
          code: 'custom',
          message: 'EMAIL_FROM must use an authorized production sender',
          path: ['EMAIL_FROM'],
        });
      }
    }
  ...
  const origins = value
    ...
    .map((entry) => {
      ...
      if (url.origin !== entry.replace(/\/$/, '')) {
        throw new Error(
          `AUTH_TRUSTED_ORIGINS entry must be an origin: ${entry}`,
        );
      }
      return url.origin;
    });
...
export function loadConfig(
  ...
  if (
    ...
    (trustedOrigins.length !== 1 ||
      trustedOrigins[0] !== new URL(parsed.BETTER_AUTH_URL).origin)
  ) {
    throw new Error(
      'AUTH_TRUSTED_ORIGINS must contain only BETTER_AUTH_URL in production',
    );
  }
  const google =
