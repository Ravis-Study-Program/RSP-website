...
export function isMfaRecent(
  ...
  if (!Number.isFinite(verifiedAtMs) || verifiedAtMs > now.getTime()) return false;
...
export function buildAccessClaims(options: {
  ...
  if (!options.user.emailVerified) throw new AccessPolicyError("email_not_verified");
...
export function requiresRecentMfa(globalRoles: readonly string[], seasonRole?: string): boolean {
  return (
