...
export function nextAccountState(current: AccountState, command: LifecycleCommand): AccountState {
  const transitions: Record<AccountState, Partial<Record<LifecycleCommand, AccountState>>> = {
  ...
  if (!next) throw new Error(`invalid account lifecycle transition: ${current} -> ${command}`);
  return next;
}
