...
export function ActivityChart({
  ...
  data: Array<{ week: string; attempts: number; interviews: number }>;
}) {
  ...
  const busiest = data.reduce<(typeof data)[number] | null>(
    (current, item) =>
      !current ||
      item.attempts + item.interviews > current.attempts + current.interviews
        ? item
        : current,
    null,
  );
