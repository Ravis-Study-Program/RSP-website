...
export function ProfilePage() {
  ...
  const canViewMemberHistory = Boolean(
    user.data &&
  ...
  if (publicProfile.isLoading || user.isLoading || seasons.isLoading)
    return <PageSkeleton label="Loading profile" />;
  if (publicProfile.isError || user.isError || seasons.isError)
    return (
    ...
    <div className={styles.page}>
      <div className={styles.profileBanner}>
        <div className={styles.profileHeader}>
          ...
          <div>
            <h1>{displayName}</h1>
            <p>
              @{savedSlug ?? person.slug} · {person.season}
            </p>
          </div>
        </div>
      </div>
      ...
      {isSelf ? (
        <InlineNotice tone="success">
          ...
          below are only visible to you and authorised programme administrators.
        </InlineNotice>
      ) : null}
      ...
      >
        ...
        <div className={styles.panel}>
          ...
          <p className={styles.helper}>
            Detailed private notes and scores remain visible only to authorised
            relationships. The API currently exposes the safe aggregate for
            another member.
          </p>
        </div>
      </section>
...
function EditProfileDialog({
  ...
  return (
    <FormDialog
      title="Edit profile"
      description="Your name and slug are visible to approved active members and alumni."
      open={open}
