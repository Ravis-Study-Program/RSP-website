...
// Store owns the GORM handle and its underlying database/sql pool. Goose,
...
func (p *Store) Close() error { return p.SQL.Close() }

func (p *Store) Ping(ctx context.Context) error { return p.SQL.PingContext(ctx) }
...
func (p *Store) PinnedConnection(ctx context.Context) (*gorm.DB, func() error, error) {
...
func (p *Store) ObservabilitySnapshot(ctx context.Context) (observability.Snapshot, error) {
	pool := p.SQL.Stats()
...
func (p *Store) classifyRevision(ctx context.Context, db *gorm.DB, query string, args ...any) error {
	var revision int64
