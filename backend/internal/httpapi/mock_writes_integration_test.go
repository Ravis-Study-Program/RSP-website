...
func TestMockWritesPreserveRoundMetadataAndHistory(t *testing.T) {
	...
	if _, err := fixture.db.GetMockInterview(context.Background(), interview.ID); !errors.Is(err, postgres.ErrNotFound) {
...
func TestMockWriteGuardsLeaveDataAndHistoryUnchanged(t *testing.T) {
	...
	interview.Revision++
	if _, err := fixture.db.UpdateMockInterview(context.Background(), interview, studentID, time.Now()); !errors.Is(err, postgres.ErrConflict) {
...
func TestConcurrentMockEditsAppendOnlyOneVersion(t *testing.T) {
	...
	if !(first == nil && errors.Is(second, postgres.ErrConflict)) && !(second == nil && errors.Is(first, postgres.ErrConflict)) {
