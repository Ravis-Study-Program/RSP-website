...
func TestPrivateSettingsUseHistoricalRelationshipPolicy(t *testing.T) {
	...
	if err := fixture.db.DB.Exec(`UPDATE app.enrollments SET state='withdrawn',assignment_state='revoked',activated_at=NULL WHERE id=?`, studentMemberID).Error; err != nil {
		t.Fatal(err)
	}
	request(http.StatusNotFound)
	if err := fixture.db.DB.Exec(`UPDATE app.enrollments SET state='active',assignment_state='active',activated_at=now() WHERE id=?`, studentMemberID).Error; err != nil {
		t.Fatal(err)
	}
	...
	request(http.StatusNotFound)
	if err := fixture.db.DB.Exec(`INSERT INTO app.mentorships(id,season_id,mentor_enrollment_id,student_enrollment_id)
		VALUES ('00000000-0000-7000-8000-000000000110',?,?,?)`, seasonID, otherMemberID, studentMemberID).Error; err != nil {
		t.Fatal(err)
	}
	request(http.StatusOK)
	if err := fixture.db.DB.Exec(`UPDATE app.mentorships SET ended_at=now() WHERE season_id=?`, seasonID).Error; err != nil {
		t.Fatal(err)
	}
	request(http.StatusNotFound)
}
